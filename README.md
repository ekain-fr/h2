...
h2 manages AI coding agents (currently Claude Code) as background processes, lets them message each other and you, and coordinates teams of agents working on projects together. It's a 3-tier system — use as much or as little as you need.
...
`h2 list` shows each agent's real-time state — active, idle, thinking, in tool use, waiting on permission, compacting — so you can tell at a glance what's happening:
...
This creates `~/.h2/` with default configuration, roles, and hooks.
...
Launches Claude Code for you to log in. Credentials are stored in `~/.h2/claude-config/default/`.
...
~/.h2/
  ...
  claude-config/default/    # Claude auth + settings + hooks
